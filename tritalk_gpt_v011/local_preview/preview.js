const MODEL_MANIFEST_URL = "./model/tritalk-gpt-v011-browser-model.json";

const promptInput = document.getElementById("tritalk-v011-prompt");
const generateButton = document.getElementById("tritalk-v011-generate-btn");
const stopButton = document.getElementById("tritalk-v011-stop-btn");
const outputEl = document.getElementById("tritalk-v011-output");
const statusEl = document.getElementById("tritalk-v011-status");
const metaEl = document.getElementById("tritalk-v011-meta");
const temperatureInput = document.getElementById("tritalk-v011-temperature");
const topKInput = document.getElementById("tritalk-v011-topk");
const tokenCountInput = document.getElementById("tritalk-v011-token-count");
const samplePromptButtons = Array.from(document.querySelectorAll("[data-local-prompt]"));
...
async function generateCompletion() {
  ...
  try {
    ...
    const maxNewTokens = Math.max(1, Number(tokenCountInput.value) || 45);
    const temperature = Math.max(0.1, Number(temperatureInput.value) || 0.85);
    const topK = Math.max(1, Number(topKInput.value) || 30);
    ...
    console.error(error);
    setStatus("The local preview hit an error.", "bad");
  } finally {
    cancelGeneration = false;
    setBusy(false);
  }
...
function bindPromptChips() {
  for (const button of samplePromptButtons) {
    button.addEventListener("click", () => {
      promptInput.value = button.dataset.localPrompt || "";
      promptInput.focus();
    });
  }
}
...
async function loadModel() {
  setStatus("Loading local model manifest…", "working");
  ...
  const weightsUrl = `./model/${manifest.weights_file}`;
  setStatus("Downloading local browser weights…", "working");
  ...
  metaEl.textContent =
    ...
    `${sizeMb} MB weights`;
  setStatus("Model loaded. This local preview is now running entirely in your browser.", "good");
  setBusy(false);
}
...
setBusy(true);
setOutput("");
setStatus("Preparing local preview…", "neutral");

loadModel().catch((error) => {
  console.error(error);
  setStatus("Could not load the local preview model.", "bad");
  metaEl.textContent = "Try exporting the v0.1.1 browser bundle again.";
  setBusy(true);
});
